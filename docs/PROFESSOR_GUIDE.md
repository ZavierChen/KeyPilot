...
本项目的评阅目标是验证三个可以分开观察的能力：Windows 助手的规则与受控操作、更换本地模型后的结构化路由、自定义语音包的导入与合成。源码和测试提供可复现路径；模型理解准确率、真人语音质量与真实硬件延迟需要在指定环境中另做实验，不把单元测试结果当作这些指标。

项目采用 AI 辅助开发：作者主导系统架构、运行逻辑、功能需求与迭代选择，大量代码由 AI 工具辅助实现。设计过程及失败后的修正见 [版本历史](VERSION_HISTORY.md)，依据见 [历史来源](HISTORY_SOURCES.md)。声音包只在本机使用，提交材料不附带任何私人声音资源。
...
保持初始的联网关闭、复杂代理关闭、语音自动执行关闭、朗读关闭。输入“打开计算器”，手动点击执行，观察计算器窗口与应用回执。此实验主要验证规则与 Windows 执行层，不能作为模型推理结果。
...
- 当前文档提供实验方法，不预先宣称在教授设备上的准确率、推理速度或音色质量。实际验收结论应附环境与记录。
