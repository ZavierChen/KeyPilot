...
**本项目的语音包只在本地使用，不上传 GitHub，也不随教授交付包分发。本文规范的是如何在自己的电脑组装和使用语音包；下面的导入是本机目录之间的复制，不是上传。**
...
向教授报告时可保存 `validate --python` 的 JSON 输出、公开权重来源、自己下载文件的 SHA256（PowerShell `Get-FileHash`）、机器 CPU/RAM 和手工试听结果。依赖/模型加载校验、真实音频合成、音质评价应分别记录，不能把 mock 单测写成真实声音复现。

本仓库的导入单测使用临时假文件和 mock 运行时，覆盖布局、拒绝执行字段、路径越界、缺失资源、重名、失败清理及发布 `ready` 的条件；不携带真人音频，也不替代上面的真实模型验证。
