...
def default_handoff_path() -> Path:
    return Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "KeyPilot" / "chat-handoff.json"


def default_marvis_handoff_path() -> Path:
    return Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "KeyPilot" / "marvis-handoff.json"
...
def _handoff_lock() -> Iterator[None]:
    ...
    handle = kernel32.CreateMutexW(None, False, r"Local\KeyPilot.ChatHandoff")
    if not handle:
