...
class Message:
    ...
    def decode(cls, data: bytes) -> "Message":
        if len(data) < 4:
            raise ValueError("火山引擎响应短于 4 字节协议头。")
        ...
        msg = cls(msg_type=msg_type, flag=flag)

        if msg_type == MsgType.ERROR:
            raw = stream.read(4)
            if len(raw) == 4:
                msg.error_code = struct.unpack(">I", raw)[0]
        elif flag in (Flag.POSITIVE_SEQUENCE, Flag.NEGATIVE_SEQUENCE):
            msg.sequence = struct.unpack(">i", stream.read(4))[0]

        if flag == Flag.WITH_EVENT:
            event_value = struct.unpack(">i", stream.read(4))[0]
            try:
                msg.event = Event(event_value)
            except ValueError:
                msg.event = event_value
            if msg.event not in _CONNECTION_EVENTS:
                msg.session_id = _read_string(stream)
            if msg.event in {
                Event.CONNECTION_STARTED,
                Event.CONNECTION_FAILED,
                Event.CONNECTION_FINISHED,
            }:
                msg.connect_id = _read_string(stream)

        size_raw = stream.read(4)
        if len(size_raw) == 4:
            size = struct.unpack(">I", size_raw)[0]
            msg.payload = stream.read(size)
        return msg
