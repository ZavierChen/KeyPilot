...
from .reply_length import reply_instruction
...
class OllamaBridge:
    ...
    def answer(self, utterance: str, context: str = "") -> OllamaAnswer | None:
        ...
        system = (
            ...
            "回答要自然，适合直接朗读。你不能访问互联网，也不知道实时状态。"
        ...
        system += reply_instruction(getattr(self, "reply_length", "低"))
        if context.strip():
            user_content = f"最近对话：\n{context}\n\n当前问题：{utterance}"
        response = self._post(
            {
                ...
                "think": False,
                "keep_alive": self.keep_alive,
                "options": {"temperature": 0.2, "num_ctx": 4096, "num_predict": {"低": 220, "中": 600, "高": 1600}.get(getattr(self, "reply_length", "低"), 220)},
            }
        )
    ...
    def answer_with_context(
        ...
        system = (
            ...
            "只输出规定的 JSON 字段。"
            + reply_instruction(getattr(self, "reply_length", "低"))
        )
        response = self._post(
            {
                "model": self.model,
                ...
                "think": False,
                "keep_alive": self.keep_alive,
                "options": {"temperature": 0.1, "num_ctx": 4096, "num_predict": {"低": 260, "中": 600, "高": 1600}.get(getattr(self, "reply_length", "低"), 260)},
            }
        )
        content = response.get("message", {}).get("content", "")
        if not isinstance(content, str):
