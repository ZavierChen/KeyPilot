...
class AlreadyRunningError(RuntimeError):
    """Another launcher won the race to start the resident service."""
...
def acquire_single_instance() -> object:
    ...
    handle = kernel32.CreateMutexW(None, False, "Local\\KeyPilot.Repro.CopilotKeyAssistant")
    if not handle:
        raise ctypes.WinError()
    if kernel32.GetLastError() == ERROR_ALREADY_EXISTS:
        kernel32.CloseHandle(handle)
        raise AlreadyRunningError("KeyPilot is already running")
    return handle
