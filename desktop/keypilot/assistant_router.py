...
class LocalCommandRouter:
    ...
    def _route_browser_read(utterance: str, text: str) -> SkillCall | None:
        ...
        google_read = re.match(r"^(?:请|帮我)?(?:读(?:取|一下)?|朗读|念(?:一下)?)\s*(?:google|谷歌)(?:\s*(?:ai|AI))?(?:的)?(?:页面|网页|内容|总结|回答|简介)?[。！？?!]*$", text, re.I)
        if google_read or (any(marker in text for marker in browser_markers) and any(
